...
class SparseLinear(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.weight = nn.Parameter(torch.randn(out_features, in_features) / math.sqrt(out_features))
        if bias:
    ...
    def forward(self, input):
        out = torch.sparse.mm(input, torch.transpose(self.weight, 0, 1))
        if self.bias is not None:
...
class DenseInputNet(torch.nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.net_freq = nn.Sequential(
                nn.Linear(conf.input_size, conf.hidden_sizes[0])
                )
...
class IntermediateNet(torch.nn.Module):
    def __init__(self, conf):
        ...
        for i in range(len(conf.hidden_sizes) - 1):
            self.intermediate_net.add_module(name="hidden_layer_%d"%i, module=nn.Sequential(
                nn.ReLU(),
                nn.Dropout(conf.hidden_dropout),
                nn.Linear(conf.hidden_sizes[i], conf.hidden_sizes[i+1]),
            ))
    def forward(self, H):
        return self.intermediate_net(H)
...
class Trunk(torch.nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.trunk = SparseInputNet(conf)

        self.apply(self.init_weights)

    def init_weights(self, m):
        if type(m) in [SparseLinear]:
            torch.nn.init.xavier_uniform_(m.weight)
            m.bias.data.fill_(0.01)
